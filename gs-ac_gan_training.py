...
def init_dataset(hapt_genotypes_path: str, extra_data_path: str, target_column: str, minimum_samples: int):
    ...
    class_to_pca.loc[:, "pca_sum"] = X_pca.sum(axis=1)
    sorted_by_pca = class_to_pca.groupby(target_column).sum()["pca_sum"].sort_values().to_dict()
    class_to_id = {class_name: class_id for class_id, class_name in enumerate(sorted_by_pca.keys())}
...
# Make discriminator
def build_discriminator(number_of_genotypes: int, num_classes: int, alph: float):
...
def main(hapt_genotypes_path: str, extra_data_path: str, experiment_results_path: str, latent_size: int, alph: float,
         ...
         minimum_samples: int, first_epoch: int, target_column: str, sequence_results_path: str):
    K.clear_session()
    init_gpus()
    ...
    discriminator.compile(optimizer=Adam(lr=d_learn), loss=['binary_crossentropy', 'mse'],
    ...
    acgan.compile(optimizer=Adam(lr=g_learn), loss=['binary_crossentropy', 'mse'], loss_weights=[1, class_loss_weights],
                  metrics=['accuracy'])
